...
const Subjects = () => {
    ...
    const handleDelete = () => {
        selectedRows.map((id) => DELETE_fetchRequest(id, 'subjects'))
    ...
    const handleSaveStudent = (student) => {
        ...
        student.id ?
            data = PUT_fetchRequest(student, 'subjects')
            :
            data = POST_fetchRequest(student, 'subjects')
    ...
    const columns = [
        { sortable: true, width: '100%', key: "fullName", label: "Предмет", type: "text" },
    ];

    return (
        <>
            <PageHeader onAdd={() => handleOpenModal()} title={'Предметы'} />
